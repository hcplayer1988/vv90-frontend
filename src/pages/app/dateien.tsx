...
// Client-side pre-check, mirrors the limits from
// dateien/api/serializers.py (ALLOWED_EXTENSIONS, MAX_DATEI_GROESSE_MB).
// The backend validates again server-side as well.
...
// All popups (new folder, rename, delete, move) go through this single
// state instead of several boolean flags - that way only one dialog can
// ever be open at once, and each dialog carries exactly the data it needs.
type ModalState =
  | { type: 'neuerOrdner' }
...
/** Builds the path from the top level down to the current folder (for the
 *  breadcrumb bar), by walking the parent chain backwards. */
function baueOrdnerPfad(alleOrdner: Ordner[], aktuelleId: number | null): Ordner[] {
  const pfad: Ordner[] = []
  let id = aktuelleId
  while (id !== null) {
 ...
 * documents), organizable into arbitrarily deep nested folders.
...
function Dateien() {
  ...
  const [aktuellerOrdnerId, setAktuellerOrdnerId] = useState<number | null>(null)
  ...
  async function ladeOrdnerUndSpeicher() {
    const [ordnerData, speicherData] = await Promise.all([listAlleOrdner(), getSpeicher()])
  ...
  const ordnerPfad = useMemo(() => baueOrdnerPfad(alleOrdner, aktuellerOrdnerId), [alleOrdner, aktuellerOrdnerId])
  ...
  async function handleFileChange(event: React.ChangeEvent<HTMLInputElement>) {
    ...
    // Reset the input value so onChange also fires when the same file is
    ...
    if (file.size > MAX_DATEI_GROESSE_MB * 1024 * 1024) {
      setUploadError(`Die Datei darf maximal ${MAX_DATEI_GROESSE_MB}MB groß sein.`)
      return
    }
    ...
    try {
      ...
      // Reload the list and storage display instead of guessing locally -
      // that way the quota display always matches exactly what the
      // backend computed.
      await ladeAlles(aktuellerOrdnerId)
    } catch (err) {
      setUploadError(extractFirstError(err, 'Datei konnte nicht hochgeladen werden.'))
    } finally {
  ...
  function toggleSelectAll() {
    setSelectedIds((prev) => (prev.size === dateien.length ? new Set() : new Set(dateien.map((d) => d.id))))
  }
  ...
  async function handleModalSubmit() {
    ...
    if (modal.type === 'neuerOrdner') {
      ...
      try {
        ...
        setModalError(extractFirstError(err, 'Ordner konnte nicht erstellt werden.'))
        setIsModalBusy(false)
      }
      return
    }
    ...
    if (modal.type === 'ordnerUmbenennen') {
      ...
      try {
        ...
        setModalError(extractFirstError(err, 'Ordner konnte nicht umbenannt werden.'))
        setIsModalBusy(false)
      }
      return
    }
    ...
    if (modal.type === 'ordnerLoeschen') {
      ...
      try {
        ...
        setModalError(extractFirstError(err, 'Ordner konnte nicht gelöscht werden.'))
        setIsModalBusy(false)
      }
      return
    }
  ...
  async function verschiebeAusgewaehlteDateien(zielOrdnerId: number | null) {
    ...
    setModalError(null)
    try {
      await Promise.all(modal.ids.map((id) => verschiebeDatei(id, zielOrdnerId)))
      setSelectedIds(new Set())
      await ladeAlles(aktuellerOrdnerId)
      schliesseModal()
    } catch {
  ...
  const belegtProzent = speicher ? Math.min((speicher.genutzt_mb / speicher.quota_mb) * 100, 100) : 0
    ...
    <div>
      <div className="view-header">
        <div>
          ...
          <h1>Meine Dateien</h1>
          <p>Nur du siehst diese Dateien. Teilen mit anderen Mitgliedern kommt in einem späteren Update.</p>
        </div>
      </div>
      ...
      <div className="member-card">
        {speicher && (
          <div style={{ marginBottom: '20px' }}>
            ...
            <p style={{ fontSize: '13px', color: 'var(--muted)', marginTop: '6px' }}>
              {speicher.genutzt_mb}MB von {speicher.quota_mb}MB belegt ({speicher.frei_mb}MB frei)
            </p>
          </div>
        )}
 
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '10px', marginBottom: '6px' }}>
          <label className="btn-outline" style={{ cursor: isUploading ? 'default' : 'pointer' }}>
        ...
        <p style={{ fontSize: '11px', color: 'var(--muted)', marginBottom: '16px' }}>
          PDF, Bilder, Word/OpenDocument/TXT/Excel, max. {MAX_DATEI_GROESSE_MB}MB pro Datei.
        </p>
 
        {uploadError && <p style={{ fontSize: '13px', color: '#c8102e', marginBottom: '12px' }}>{uploadError}</p>}
        {loadError && <p style={{ fontSize: '13px', color: '#c8102e', marginBottom: '12px' }}>{loadError}</p>}
        ...
        >
          ...
          {ordnerPfad.map((o) => (
            <span key={o.id} style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
            ...
            {unterordner.map((o) => (
              ...
              >
                ...
                <div style={{ display: 'flex', gap: '8px' }}>
                  <button className="btn-outline" onClick={() => oeffneUmbenennenModal(o)}>
                    Umbenennen
                  </button>
                  <button
                    className="btn-outline"
        ...
        {/* Bulk action bar, appears as soon as at least one file is
            checked */}
        {selectedIds.size > 0 && (
          <div
            style={{
          ...
          >
            <span style={{ fontSize: '13px' }}>{selectedIds.size} ausgewählt</span>
            <button
              className="btn-outline"
              onClick={() => {
                setModalError(null)
                setModal({ type: 'dateienVerschieben', ids: Array.from(selectedIds) })
              }}
            >
              Verschieben
            </button>
            <button
              ...
              onClick={() => {
                setModalError(null)
                setModal({ type: 'dateienLoeschen', ids: Array.from(selectedIds) })
              }}
            >
              Löschen
            </button>
        ...
        {dateien.length === 0 && unterordner.length === 0 ? (
          <p style={{ marginTop: '20px', color: 'var(--muted)' }}>Dieser Ordner ist noch leer.</p>
        ) : (
          dateien.length > 0 && (
            <>
              <label
              ...
              >
                <input type="checkbox" checked={selectedIds.size === dateien.length} onChange={toggleSelectAll} />
                Alle auswählen
              </label>
      ...
      {(modal?.type === 'neuerOrdner' || modal?.type === 'ordnerUmbenennen') && (
        <ModalOverlay onClose={schliesseModal}>
          <h3 style={{ marginTop: 0 }}>{modal.type === 'neuerOrdner' ? 'Neuer Ordner' : 'Ordner umbenennen'}</h3>
          <input
            type="text"
            value={modalInput}
          ...
          />
          {modalError && <p style={{ fontSize: '13px', color: '#c8102e', marginBottom: '12px' }}>{modalError}</p>}
          <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '10px' }}>
            <button className="btn-outline" onClick={schliesseModal} disabled={isModalBusy}>
              Abbrechen
            </button>
            <button className="btn-outline" onClick={handleModalSubmit} disabled={isModalBusy}>
      ...
      {modal?.type === 'ordnerLoeschen' && (
        <ModalOverlay onClose={schliesseModal}>
          ...
          <p>
            Soll der Ordner „{modal.ordner.name}“ wirklich gelöscht werden? Alle enthaltenen Unterordner und
            Dateien werden dabei unwiderruflich mitgelöscht.
          </p>
          {modalError && <p style={{ fontSize: '13px', color: '#c8102e', marginBottom: '12px' }}>{modalError}</p>}
          <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '10px' }}>
            <button className="btn-outline" onClick={schliesseModal} disabled={isModalBusy}>
      ...
      {modal?.type === 'dateienVerschieben' && (
        <ModalOverlay onClose={schliesseModal}>
          <h3 style={{ marginTop: 0 }}>
            {modal.ids.length === 1 ? 'Datei verschieben' : `${modal.ids.length} Dateien verschieben`}
          </h3>
          <p style={{ fontSize: '13px', color: 'var(--muted)' }}>Zielordner auswählen:</p>
          {modalError && <p style={{ fontSize: '13px', color: '#c8102e', marginBottom: '12px' }}>{modalError}</p>}
